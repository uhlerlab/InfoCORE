...
if __name__ == "__main__":
    ...
    ogbdatapath = '/home/wangchy/InfoCORE/GE/data/ogb_data'
    ...
    cells = ['A375', 'A549', 'HA1E', 'HCC515', 'HT29', 'MCF7', 'PC3', 'VCAP', 'HEPG2']
    ...
    for iter in range(args.n_iter):
            ...
            for epoch in range(args.n_epoches):
                ...
                with torch.no_grad():
                    ...
                    if ((args.criterion=='auc' and val_result_dict['rocauc'] > best_val_auc) or (args.criterion=='loss' and val_loss < best_loss)):
                        ...
                        best_loss = val_loss
                        best_epoch = epoch
                        os.makedirs(f'/home/wangchy/InfoCORE/GE/ftmodel_save/{args.druglistname}/{args.model_dir}_{args.lr}_{args.n_epoches}', exist_ok=True)
                        torch.save(ftmodel.state_dict(), os.path.join(f'/home/wangchy/InfoCORE/GE/ftmodel_save/{args.druglistname}/{args.model_dir}_{args.lr}_{args.n_epoches}', f'model_{nfold}.tar'))
        ...
        for nfold in range(5):
            ftmodel.load_state_dict(torch.load(os.path.join(f'/home/wangchy/InfoCORE/GE/ftmodel_save/{args.druglistname}/{args.model_dir}_{args.lr}_{args.n_epoches}', f'model_{nfold}.tar')))
            ftmodel.eval()
            with torch.no_grad():
                val_loss = 0.0
                val_preds = []; val_labels = []
